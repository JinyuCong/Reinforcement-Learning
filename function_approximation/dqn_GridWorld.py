...
def dqn(env, num_episodes=1000, lr=1e-3, gamma=0.9,
    ...
    for ep in range(num_episodes):
        ...
        while not done and ep_step < 200:
            state_tensor = state_to_tensor(state, env.env_size)
            ...
            W, H = env.env_size[0], env.env_size[1]
            state_array = np.array([X/(W-1), Y/(H-1)])
            next_state_array = np.array([next_X/(W-1), next_Y/(H-1)])
            buffer.push(state_array, action_idx, reward, next_state_array, done)
